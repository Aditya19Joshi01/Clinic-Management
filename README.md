...
Core to this system is strict data isolation. We verify this via **Automated Integration Tests** (`backend/tests/test_isolation.py`).

### Verification Method
Every API query is automatically filtered by the authenticated user's `company_id`.
```python
# Example from backend/app/routers/patients.py
patients = db.query(models.Patient).filter(
    models.Patient.company_id == current_user.company_id  # <--- Forced Filter
).all()
```
**Test Case Proof:**
1.  **Company A** creates "Patient A".
2.  **Company B** tries to fetch "Patient A".
3.  Result: `404 Not Found` (Data is invisible to Company B).
...
-   **Swagger UI**: [http://localhost:8000/docs](http://localhost:8000/docs) - Test endpoints interactively.
